        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <div class="container">
                        <h1 class="text-3xl font-extrabold text-primary-header mb-6">{{ __('app.edit') }} {{ $usuario->name }}</h1>
                        ...
                        <form action="{{ route('usuarios.update', $usuario->id) }}" method="POST">
                            ...
                            <div class="mb-4">
                                ...
                                <input type="text" class="form-input mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" id="name" name="name" value="{{ old('name', $usuario->name) }}" required>
                            </div>

                            <div class="mb-4">
                                <label for="email" class="block text-sm font-medium text-gray-700 mb-1">{{ __('app.email') }}</label>
                                <input type="email" class="form-input mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" id="email" name="email" value="{{ old('email', $usuario->email) }}" required>
                            </div>
                            ...
                            <div class="mb-4">
                                ...
                                <p class="mt-1 text-sm text-gray-500">{{ __('Dejar en blanco para no cambiar la contraseña') }}.</p>
                            </div>
                            ...
                            <div class="mb-4">
                                ...
                                <select id="rol" name="rol" class="form-select mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required>
                                    ...
                                    <option value="bibliotecario" {{ old('rol', $usuario->rol) == 'bibliotecario' ? 'selected' : '' }}>Bibliotecario</option>
                            ...
                            <div class="mb-4">
                                ...
                                <select id="universidad_id" name="universidad_id" class="form-select mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required>
                                    ...
                                    @foreach($universidades as $universidad)
                                        <option value="{{ $universidad->id }}" {{ old('universidad_id', $usuario->universidad_id) == $universidad->id ? 'selected' : '' }}>{{ $universidad->name }}</option>
                                    @endforeach
                                </select>
                            </div>
