                ...
                <div class="p-6 text-gray-900">
                    <div class="container-fluid px-4">
                        <div class="flex justify-between items-center mb-4">
                            <h2 class="text-3xl font-extrabold text-primary-header">{{ __('app.edit_student') }}</h2>
                            ...
                            <form action="{{ route('estudiantes.update', $estudiante) }}" method="POST">
                                    ...
                                    <div>
                                        <div class="mb-4">
                                            ...
                                            <select class="form-select mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 @error('carrera') border-red-500 @enderror" 
                                                ...
                                                <option value="Ingeniería de Sistemas" {{ old('carrera', $estudiante->carrera) == 'Ingeniería de Sistemas' ? 'selected' : '' }}>Ingeniería de Sistemas</option>
                                                <option value="Ingeniería Civil" {{ old('carrera', $estudiante->carrera) == 'Ingeniería Civil' ? 'selected' : '' }}>Ingeniería Civil</option>
                                                <option value="Ingeniería Industrial" {{ old('carrera', $estudiante->carrera) == 'Ingeniería Industrial' ? 'selected' : '' }}>Ingeniería Industrial</option>
                                                <option value="Arquitectura" {{ old('carrera', $estudiante->carrera) == 'Arquitectura' ? 'selected' : '' }}>Arquitectura</option>
                                                <option value="Derecho" {{ old('carrera', $estudiante->carrera) == 'Derecho' ? 'selected' : '' }}>Derecho</option>
                                                <option value="Medicina" {{ old('carrera', $estudiante->carrera) == 'Medicina' ? 'selected' : '' }}>Medicina</option>
                                            </select>
                                            @error('carrera')
                                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                            @enderror
                                ...
                                <div class="mb-4">
                                    ...
                                    <select class="form-select mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 @error('universidad_id') border-red-500 @enderror" 
                                        ...
                                        @foreach($universidades as $universidad)
                                            <option value="{{ $universidad->id }}" {{ old('universidad_id', $estudiante->universidad_id) == $universidad->id ? 'selected' : '' }}>{{ $universidad->name }}</option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="flex justify-end gap-2 mt-4">
                                    ...
                                    <button type="submit" class="btn-confirm px-4 py-2 rounded-md font-semibold text-xs uppercase tracking-widest">
                                        <i class="fas fa-save mr-2"></i> {{ __('app.update_student') }}
                                    </button>
                                </div>
                            </form>
                        </div>
