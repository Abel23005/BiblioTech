<x-app-layout>
    <x-slot name="header">
        <h1 class="text-3xl font-bold text-blanco-humo">@lang('app.edit_author')</h1>
    </x-slot>
        ...
        <div class="bg-white p-6 rounded-lg shadow">
            <form action="{{ route('autores.update', $autor->id) }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="nombre" id="nombre" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('nombre') border-red-500 @enderror" value="{{ old('nombre', $autor->nombre) }}" required>
                    @error('nombre')
                ...
                <div class="mb-6">
                    ...
                    <textarea name="biografia" id="biografia" rows="5" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('biografia') border-red-500 @enderror">{{ old('biografia', $autor->biografia) }}</textarea>
                    @error('biografia')
                        <p class="text-red-500 text-xs italic mt-2">{{ $message }}</p>
                    @enderror
                </div>
