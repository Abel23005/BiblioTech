                ...
                <div class="p-6 text-gray-900">
                    <div class="container-fluid px-4">
                        <div class="flex justify-between items-center mb-4">
                            <h2 class="text-3xl font-extrabold text-primary-header">{{ __('app.edit') }} {{ __('app.book') }}</h2>
                            ...
                            <form action="{{ route('libros.update', $libro) }}" method="POST">
                                    ...
                                    <div>
                                        <div class="mb-4">
                                            ...
                                            <select class="form-select mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 @error('categoria') border-red-500 @enderror" 
                                                ...
                                                <option value="Ficción" {{ old('categoria', $libro->categoria) == 'Ficción' ? 'selected' : '' }}>{{ __('app.fiction') }}</option>
                                                <option value="No Ficción" {{ old('categoria', $libro->categoria) == 'No Ficción' ? 'selected' : '' }}>{{ __('app.non_fiction') }}</option>
                                                <option value="Ciencia" {{ old('categoria', $libro->categoria) == 'Ciencia' ? 'selected' : '' }}>{{ __('app.science') }}</option>
                                                <option value="Tecnología" {{ old('categoria', $libro->categoria) == 'Tecnología' ? 'selected' : '' }}>{{ __('app.technology') }}</option>
                                                <option value="Historia" {{ old('categoria', $libro->categoria) == 'Historia' ? 'selected' : '' }}>{{ __('app.history') }}</option>
                                                <option value="Arte" {{ old('categoria', $libro->categoria) == 'Arte' ? 'selected' : '' }}>{{ __('app.art') }}</option>
                                            </select>
                                            @error('categoria')
                                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                            @enderror
                                    ...
                                    <div>
                                        <div class="mb-4">
                                            ...
                                            <select class="form-select mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 @error('estado') border-red-500 @enderror" 
                                                    ...
                                                    name="estado" 
                                                    required>
                                                <option value="nuevo" {{ old('estado', $libro->estado) == 'nuevo' ? 'selected' : '' }}>{{ __('app.new') }}</option>
                                                <option value="bueno" {{ old('estado', $libro->estado) == 'bueno' ? 'selected' : '' }}>{{ __('app.good') }}</option>
                                                <option value="regular" {{ old('estado', $libro->estado) == 'regular' ? 'selected' : '' }}>{{ __('app.regular') }}</option>
                                                <option value="deteriorado" {{ old('estado', $libro->estado) == 'deteriorado' ? 'selected' : '' }}>{{ __('app.deteriorated') }}</option>
                                            </select>
                                            @error('estado')
                                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                            @enderror
                                ...
                                <div>
                                    <div class="mb-4">
                                        <div class="flex items-center">
                                            <input type="checkbox" 
                                                   class="form-checkbox h-5 w-5 text-indigo-600 rounded @error('disponible') border-red-500 @enderror" 
                                                   id="disponible" 
                                                   name="disponible" 
                                                   value="1"
                                                   {{ old('disponible', $libro->disponible) ? 'checked' : '' }}>
                                            <label class="ml-2 block text-sm font-medium text-gray-700" for="disponible">{{ __('app.available_for_loan') }}</label>
                                            @error('disponible')
                                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                            @enderror
                                        </div>
                                ...
                                <div class="flex justify-end gap-2 mt-4">
                                    ...
                                    <button type="submit" class="btn-confirm px-4 py-2 rounded-md font-semibold text-xs uppercase tracking-widest">
                                        <i class="fas fa-save mr-2"></i> {{ __('app.update_book') }}
                                    </button>
                                </div>
                            </form>
                        </div>
